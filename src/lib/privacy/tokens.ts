...
export async function requestTokenBatch(
  ...
  const publicKey = await crypto.subtle.importKey(
    'spki',
    issuerPublicKeyBytes.buffer.slice(
      issuerPublicKeyBytes.byteOffset,
      issuerPublicKeyBytes.byteOffset + issuerPublicKeyBytes.byteLength,
    ) as ArrayBuffer,
  ...
  for (let i = 0; i < count; i++) {
    ...
    const serialized = tokenRequest.serialize();
